...
Each enemy type is described by an :class:`EnemyTypeInfo` dataclass with:
   name          – display name
   ...
   maximum       – max simultaneous instances on one map
   xp            – XP awarded on kill
   ...
   hitbox_radius – computed automatically from draw_commands in __post_init__
"""
...
@dataclass
class EnemyTypeInfo:
    """All static data for a single enemy type."""

    name: str
    environments: list[EnemyEnvironment]
    maximum: int
    xp: int
    ...
    hitbox_radius: int = field(init=False)

    def __post_init__(self) -> None:
        self.hitbox_radius = _compute_hitbox_radius(self.draw_commands)


ENEMY_TYPES: dict[str, EnemyTypeInfo] = {
    "slime": EnemyTypeInfo(
        name="Slime",
