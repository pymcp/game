...
from src.config import (
    SCREEN_W, SCREEN_H, VIEWPORT_W, VIEWPORT_H, TILE, FPS, BG, WORLD_COLS, WORLD_ROWS,
...
class Game:
    ...
    def _try_build_house(self, player):
        ...
        if not (0 <= build_col < WORLD_COLS and 0 <= build_row < WORLD_ROWS):
            return
        if self.world[build_row][build_col] != GRASS or player.inventory.get("Dirt", 0) < 20:
            return
    ...
    def update(self, dt):
        ...
        self.player1.update_mining(keys, mouse_buttons, dt, self.world, self.tile_hp,
                                    self.cam1_x, self.cam1_y, self.particles, self.floats)
        ...
        self.player2.update_mining(keys, mouse_buttons, dt, self.world, self.tile_hp,
                                    self.cam2_x, self.cam2_y, self.particles, self.floats)
        ...
        for w in self.workers:
            ...
            w.update(dt, self.world, self.tile_hp, target_player.inventory, self.particles, self.floats)
    ...
    def _update_enemies(self, dt):
        ...
        for enemy in self.enemies:
            ...
            target_x, target_y = (self.player1.x, self.player1.y) if dist1 < dist2 else (self.player2.x, self.player2.y)
            ...
            enemy.update(dt, target_x, target_y, avg_cam_x, avg_cam_y, self.world, self.particles)
    ...
    def _update_combat(self, keys, mouse_buttons, dt):
        ...
        if fire_input_p1 and self.player1.weapon_cooldown <= 0:
            ...
            self.projectiles.append(Projectile(self.player1.x, self.player1.y, self.player1.facing_dx, self.player1.facing_dy, wpn))
        ...
        if fire_input_p2 and self.player2.weapon_cooldown <= 0:
            ...
            self.projectiles.append(Projectile(self.player2.x, self.player2.y, self.player2.facing_dx, self.player2.facing_dy, wpn))
    ...
    def draw(self):
        ...
        # Left side: Player 1
        self._draw_player_view(self.player1, self.cam1_x, self.cam1_y, 0, 0, self.viewport_w, self.viewport_h)
        
        # Right side: Player 2
        self._draw_player_view(self.player2, self.cam2_x, self.cam2_y, self.viewport_w, 0, self.viewport_w, self.viewport_h)
        
        # Separator line
        pygame.draw.line(self.screen, (100, 100, 100), (self.viewport_w, 0), (self.viewport_w, screen_height), 2)

        pygame.display.flip()

    def _draw_player_view(self, player, cam_x, cam_y, screen_x, screen_y, view_w, view_h):
        ...
        if cam_x < 0:
            ...
            pygame.draw.rect(self.screen, border_outer, (screen_x, screen_y, border_width, view_h))
            # Add decorative bricks
            for by in range(screen_y, screen_y + view_h, 16):
                pygame.draw.rect(self.screen, border_inner, (screen_x + 2, by, border_width - 4, 8))
        ...
        if cam_x + view_w > world_pixel_w:
            ...
            pygame.draw.rect(self.screen, border_outer, (border_start, screen_y, border_width, view_h))
            # Add decorative bricks
            for by in range(screen_y, screen_y + view_h, 16):
                pygame.draw.rect(self.screen, border_inner, (border_start + 2, by, border_width - 4, 8))
        
        # Top border
        if cam_y < 0:
            border_height = min(view_h, int(-cam_y) + screen_y)
            pygame.draw.rect(self.screen, border_outer, (screen_x, screen_y, view_w, border_height))
            # Add decorative bricks
            for bx in range(screen_x, screen_x + view_w, 16):
                pygame.draw.rect(self.screen, border_inner, (bx, screen_y + 2, 8, border_height - 4))
        ...
        if cam_y + view_h > world_pixel_h:
            ...
            pygame.draw.rect(self.screen, border_outer, (screen_x, border_start, view_w, border_height))
            # Add decorative bricks
            for bx in range(screen_x, screen_x + view_w, 16):
                pygame.draw.rect(self.screen, border_inner, (bx, border_start + 2, 8, border_height - 4))
    ...
    def _draw_player_ui(self, player, screen_x, screen_y, view_w, view_h):
        ...
        pygame.draw.rect(self.screen, (50, 50, 50), (screen_x + 10, screen_y + 10, bar_w, bar_h))
        pygame.draw.rect(self.screen, (0, 255, 0), (screen_x + 10, screen_y + 10, bar_w * hp_ratio, bar_h))
        pygame.draw.rect(self.screen, (255, 255, 255), (screen_x + 10, screen_y + 10, bar_w, bar_h), 2)
        hp_text = font_small.render(f"{player.hp}/{player.max_hp}", True, (255, 255, 255))
        ...
        pygame.draw.rect(self.screen, (50, 50, 0), (screen_x + 10, screen_y + 65, xp_bar_w, 10))
        pygame.draw.rect(self.screen, (255, 255, 0), (screen_x + 10, screen_y + 65, xp_bar_w * xp_ratio, 10))
        xp_text = font_small.render(f"XP: {player.xp}/{player.xp_next}", True, (255, 255, 0))
